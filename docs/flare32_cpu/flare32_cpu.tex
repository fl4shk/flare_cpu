...
\begin{document}
	...
	\begin{itemize}
		...
		\texttt{flags}, \texttt{ira}, or \texttt{ids}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Effect:  makes it so that a \textit{directly following}
			load or store instruction adds \texttt{rA} to the address to be
			loaded from or stored to.  The load or store \textit{must}
			directly follow this instruction.  Any other instructions
			directly following this one will not be affected.
			\item Note:  If the previous instruction was \texttt{index},
			the current instruction cannot be interrupted.
		\end{itemize}
		\item Opcode \texttt{0xe}
		...
		\begin{itemize}
			...
			computed as \texttt{index\_reg + \texttt{rC}}, zero-extend the
			8-bit value to 32 bits, then put the zero-extended 32-bit
			value into \texttt{rA}.
			\item The \texttt{index\_reg} value is zero in all cases except
		...
		\begin{itemize}
			\item Effect:  Load an 8-bit value from memory at address
			computed as \texttt{index\_reg + \texttt{rC}}, sign-extend the
			8-bit value to 32 bits, then put the sign-extended 32-bit
			value into \texttt{rA}.
			\item The \texttt{index\_reg} value is zero in all cases except
			the previous instruction was \texttt{index index\_reg} and if
			\texttt{index\_reg}'s value was non-zero. 
