...
class PstageIdBus:
	def __init__(self, constants):
		...
		self.sbmn_bus = mk_pstage_id_sbmn(PipeSkidBufBus)
		self.sb_fr_ex_bus = mk_pstage_id_sb_fr_ex(PipeSkidBufBus)

	def constants(self):
		return self.__constants

# Pipeline Stage ID: Instruction Decode
class PstageId(Elaboratable):
	def __init__(self, constants):
		self.__constants = constants
		self.__bus = PstageIdBus(constants=self.constants())
	...
	def elaborate(self, platform: str) -> Module:
		...
		#--------
		# to icache
		loc.m.sbmn = mk_pstage_id_sbmn(PipeSkidBuf)
		# from EX
		loc.m.sb_fr_ex = mk_pstage_id_sb_fr_ex(PipeSkidBuf)
		...
		with m.If(fr_ex_o_fwd.valid):
			with m.Switch(fr_ex_o_fwd.data.ex_outp_kind):
				with m.Case(ExOutpKind.DO_WB):
					m.d.comb += loc.state.eq(StateId.FETCH)
				with m.Case(ExOutpKind.SET_PC):
					m.d.comb += loc.state.eq(StateId.SET_PC)
				with m.Case(ExOutpKind.ICRELOAD):
					m.d.comb += loc.state.eq(StateId.ICRELOAD)
				with m.Default():
					m.d.comb += loc.state.eq(StateId.FETCH)
		with m.Else(): # If(~fr_ex_o_fwd.valid):
			m.d.comb += loc.state.eq(StateId.FETCH)

		with m.If(loc.state == StateId.SET_PC):
			m.d.sync += [
